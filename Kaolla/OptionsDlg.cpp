...
INT_PTR OptionsDlg::DialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message) {
...
void OptionsDlg::OnHScroll(HWND hwnd, HWND hwndCtl, UINT code, int pos)
{
	if (code == SB_THUMBTRACK && hwndCtl == GetDlgItem(hwnd, IDC_SWINPOS)) {
		//HWND hWndPosEdit = GetDlgItem(hwnd, IDC_EWNDPOS);
		//WCHAR szText[16] = { 0 };
		//wsprintf(szText, L"%d", pos);
		//Edit_SetText(hWndPosEdit, szText);
